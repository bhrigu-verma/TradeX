...
// IMPORTANT: Must be defined BEFORE /:ticker to avoid being matched as a ticker param
...
router.get('/:ticker', optionalAuth, (req, res) => {
    try {
      ...
      SELECT signal_type,
             ...
             SUM(correct_24h) as correct_24h,
             AVG(CASE WHEN correct_1h IS NOT NULL THEN correct_1h END) as acc_1h,
             AVG(CASE WHEN correct_4h IS NOT NULL THEN correct_4h END) as acc_4h,
             AVG(CASE WHEN correct_24h IS NOT NULL THEN correct_24h END) as acc_24h
      FROM backtest_results
    ...
    `).all(ticker);
      ...
      SELECT COUNT(*) as total,
             ...
             AVG(CASE WHEN correct_24h IS NOT NULL THEN correct_24h END) as overall_acc_24h
